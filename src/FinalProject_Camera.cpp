...
int main(int argc, const char *argv[])
{
    ...
    int dataBufferSize = 2;         // 2 images which are held in memory (ring buffer) at the same time
    vector<DataFrame> dataBuffer;   // list of dataframes held in memory at the same time
    bool result = false;
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
    {
        ...
        for(auto bo = (dataBuffer.end()-1)->boundingBoxes.begin(); bo != (dataBuffer.end()-1)->boundingBoxes.end(); bo++)
        {
        ...
        float shrinkFactor = 0.2; //0.10; // shrinks each bounding box by the given percentage to avoid 3D object merging at the edges of an ROI
        ...
        if(b3DVis)
        {
            show3DObjects((dataBuffer.end()-1)->boundingBoxes, cv::Size(10.0, 25.0), cv::Size(1000, 2000), false); // 4;20  2000;2000
        }
